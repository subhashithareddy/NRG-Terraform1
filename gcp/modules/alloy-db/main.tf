...
resource "google_alloydb_cluster" "default" {
  ...
  continuous_backup_config {
    ...
    dynamic "encryption_config" {
      for_each = var.continuous_backup_encryption_key_name != null && (var.env == "prod" || var.env == "non-prod") ? ["cont_backup_encryption_config"] : []
      content {
  ...
  for_each = var.env == "prod" && var.maintenance_update_policy != null ? [var.maintenance_update_policy] : []
  content {
    maintenance_windows {
...
resource "google_alloydb_instance" "read_pool" {
  #count         = var.create_instance ? var.number_of_read_pool_instances : 0
